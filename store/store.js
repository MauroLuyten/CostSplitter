...
class Trip {
    constructor(name,description, budget, currencies, selectedCurrency){
        this.name = name
        this.description = description
        this.budget = budget
...
class StateStore {
    ...
    addTrip(trip) {
        ...
        this.trips.set(key, new Trip(trip.name, trip.description, parseFloat(trip.budget).toFixed(2), trip.currencies, 'EUR'))
        if (this.online) {
